...
import { TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, MEMO_PROGRAM_ID, MILTON_TOKEN_ADDRESS } from '@/lib/solana/programs'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fromTokenAccount = await getAssociatedTokenAddress(MILTON_TOKEN_ADDRESS, from)
    const toTokenAccount = await getAssociatedTokenAddress(MILTON_TOKEN_ADDRESS, to)

    // Check if the sender has enough balance
    const senderBalance = await getTokenAccountBalance(connection, MILTON_TOKEN_ADDRESS, from)
