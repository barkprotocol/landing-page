...
async function fetchTokenPrice(tokenAddress: PublicKey): Promise<number> {
  try {
    ...
    if (tokenAddressString === MILTON_TOKEN_ADDRESS.toBase58()) {
...
export async function GET(request: NextRequest) {
  try {
    ...
    const [miltonInfo, usdcInfo] = await Promise.all([
      getTokenInfo(MILTON_TOKEN_ADDRESS),
      getTokenInfo(USDC_TOKEN_ADDRESS),
    ])

    const treasuryMiltonBalance = await getTokenAccountBalance(connection, MILTON_TOKEN_ADDRESS, TREASURY_WALLET_ADDRESS)
...
export async function POST(request: NextRequest) {
  try {
    ...
    const miltonInfo = await getTokenInfo(MILTON_TOKEN_ADDRESS)
    ...
    const buyerMiltonAccount = await getAssociatedTokenAddress(MILTON_TOKEN_ADDRESS, buyer)
    ...
    const treasuryMiltonAccount = await getAssociatedTokenAddress(MILTON_TOKEN_ADDRESS, TREASURY_WALLET_ADDRESS)
    transaction.add(
      createTransferCheckedInstruction(
