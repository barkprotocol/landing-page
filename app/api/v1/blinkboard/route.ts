...
import { Connection, PublicKey, Transaction, SystemProgram, LAMPORTS_PER_SOL, TransactionInstruction, sendAndConfirmTransaction, Keypair } from '@solana/web3.js'
...
const idl = JSON.parse(fs.readFileSync(path.resolve(__dirname, './blink_idl.json'), 'utf8'))
...
async function createNFT({ nftName, nftDescription, nftRoyalty, nftImage }) {
  try {
    const senderKeypair = Keypair.fromSecretKey(new Uint8Array(JSON.parse(process.env.SENDER_PRIVATE_KEY || '[]')))

    // Upload the image to Arweave
    const imageBuffer = Buffer.from(nftImage.split(',')[1], 'base64')
    ...
    const { nft } = await metaplex.nfts().create({
      uri,
      name: nftName,
      sellerFeeBasisPoints: parseFloat(nftRoyalty) * 100,
      creators: [{ address: senderKeypair.publicKey, share: 100 }],
    })

    return NextResponse.json({ message: 'NFT created successfully', nft: nft.address.toBase58(), metadata: uri })
  } catch (error) {
...
async function donate({ donationAmount, donationMessage, donationRecipient, selectedToken, fee }) {
  try {
    const result = await sendTransaction({ amount: donationAmount, recipient: donationRecipient, memo: donationMessage, selectedToken, fee })
    const data = await result.json()
