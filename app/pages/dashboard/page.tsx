...
import { ArrowLeft, Send, Gift, Shuffle, Plus, CreditCard, ImageIcon, Heart, Upload, QrCode, Zap, Wallet, BarChart, AlertCircle, Moon, Sun, RefreshCw, TrendingUp, Clock } from 'lucide-react'
import Image from 'next/image'
import { QRCodeGenerator } from '@/components/services/qr-code-generator'
...
export default function BlinkboardDashboard() {
  ...
  const calculatePortfolioValue = () => {
    ...
    for (const [token, balance] of Object.entries(tokenBalances)) {
      totalValue += balance * (marketPrices[token] || 0)
    }
    setPortfolioValue(totalValue)

    // Calculate 24h change (this is a placeholder, real implementation would require historical data)
  ...
  const portfolioData = useMemo(() => {
    return Object.entries(tokenBalances).map(([token, balance]) => ({
      name: token,
      value: balance * (marketPrices[token] || 0),
    }))
  }, [tokenBalances, marketPrices])

  return (
    <section className={`py-12 min-h-screen ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-[#f4f4f5] text-black'}`}>
      <div className="container mx-auto px-4 max-w-6xl">
        ...
        >
          ...
          <Card className="mb-8 bg-white shadow-sm dark:bg-gray-800/50">
            <CardHeader>
              ...
              <div className="flex justify-between items-center mb-4">
                <div>
                  ...
                  <p className={`text-sm ${portfolioChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                    {portfolioChange >= 0 ? '▲' : '▼'} {Math.abs(portfolioChange).toFixed(2)}% (24h)
                  </p>
                </div>
                <Button onClick={() => setShowPortfolioModal(true)}>View Details</Button>
              </div>
            ...
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                {Object.entries(marketPrices).map(([token, price]) => (
                  <motion.div
                    key={token}
                    className="flex items-center justify-between p-4 bg-white rounded-lg shadow-sm dark:bg-gray-700"
                ...
                {Object.entries(tokenBalances).map(([token, balance]) => (
                  <div key={token} className="flex justify-between items-center">
                    ...
                    <div className="text-right">
                      <p>{balance.toFixed(4)} {token}</p>
                      <p className="text-sm text-muted-foreground">${(balance * (marketPrices[token] || 0)).toFixed(2)}</p>
                    </div>
                  </div>
                ))}
                <div className="pt-4 border-t">
                  <p className="text-lg font-bold">Total Value: ${portfolioValue.toFixed(2)}</p>
                  <p className={`text-sm ${portfolioChange >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                    24h Change: {portfolioChange >= 0 ? '▲' : '▼'} {Math.abs(portfolioChange).toFixed(2)}%
                  </p>
                </div>
              </div>
            </DialogContent>
