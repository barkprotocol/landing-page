...
export default function SwapPage() {
    ...
    const fetchTokens = async () => {
      try {
        ...
        toast({
          ...
          description: "Unable to fetch the list of supported tokens. Please try again later.",
          variant: "destructive",
        })
      }
    }
  ...
  const handleSwap = async () => {
    if (!publicKey || !signTransaction || !selectedRoute) return
  ...
  const handleGetRoutes = async () => {
    ...
    try {
      ...
      const routes = await jupiter.computeRoutes({
        inputMint,
        outputMint,
        amount: inputAmount * 10 ** 9, // Convert to lamports
        slippage,
        forceFetch: true,
      })
          ...
          {tokens.map((token) => (
            <SelectItem key={token.address} value={token.symbol}>
              <div className="flex items-center">
                <div className="w-6 h-6 mr-2 rounded-full bg-[#F0E651] flex items-center justify-center">
