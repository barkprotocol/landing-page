...
import { Image as ImageIcon, Info, Zap, Repeat, CreditCard, Users, Code, Printer, ArrowRight, Play, ChevronLeft, ChevronRight, Lock, Home, Coins, FileImage } from 'lucide-react';
...
const MintNFT = () => {
  ...
  const handleMintNFT = async (event) => {
    ...
    try {
      ...
      const fee = 2.5 + 0.000005; // 2.5% + 0.000005 SOL (simulated transaction fee)
      console.log(`Minted NFT: ${nftName}, address: ${nftAddress}, fee: ${fee} SOL`);
      toast({
        title: "NFT Minted Successfully!",
        description: `NFT Address: ${nftAddress}, Fee: ${fee} SOL`,
      });
    } catch (error) {
      toast({
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-bold mb-4 text-gray-800">Mint Your Own NFT</h3>
      <Card className="bg-white text-gray-800">
        <CardHeader>
...
const MintToken = () => {
 ...
 useState('');
  ...
  const handleMintToken = async (event) => {
    ...
    setIsLoading(true);
    try {
      // Simulating token minting process
      await new Promise(resolve => setTimeout(resolve, 3000));
      const tokenAddress = PublicKey.unique().toBase58();
      const fee = 2.5 + 0.000005; // 2.5% + 0.000005 SOL (simulated transaction fee)
      console.log(`Minted ${tokenType.toUpperCase()} Token: ${tokenName} (${tokenSymbol}), address: ${tokenAddress}, supply: ${tokenSupply}, fee: ${fee} SOL`);
      toast({
        title: "Token Minted Successfully!",
        description: `Token Address: ${tokenAddress}, Fee: ${fee} SOL`,
      });
    } catch (error) {
      toast({
  ...
  return (
    <div className="space-y-6">
      <h3 className="text-2xl font-bold mb-4 text-gray-800">Mint Your Own Token</h3>
      <Card className="bg-white text-gray-800">
        <CardHeader>
...
export default function Blinkboard() {
  ...
  const tabs = [
    ...
    { id: "mint-nft", label: "Mint NFT", icon: FileImage },
    { id: "mint-token", label: "Mint Token", icon: Coins },
  ];
        ...
        <Card className="bg-white rounded-lg shadow-2xl text-gray-800">
          ...
          <CardContent className="space-y-8">
            <Tabs defaultValue={activeTab} onValueChange={setActiveTab} className="w-full">
              <TabsList className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2 bg-gray-100">
                {tabs.map((tab) => (
              ...
              <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <TabsContent value="mint-nft" className="mt-6">
                    <MintNFT />
                  </TabsContent>
                  <TabsContent value="mint-token" className="mt-6">
                    <MintToken />
                  </TabsContent>
                </motion.div>
              </AnimatePresence>
