...
export async function rateLimit(request: NextRequest, config: RateLimitConfig = defaultConfig) {
  ...
  // Increment the count and set expiry for the key
  await redis.set(key, count + 1, { px: config.window }); // px sets the expiry time in milliseconds
...
export function getRateLimitHeaders(rateLimitResult: Awaited<ReturnType<typeof rateLimit>>) {
  return {
    ...
    'X-RateLimit-Reset': rateLimitResult.reset.toUTCString() // Reset time in UTC
  };
}
